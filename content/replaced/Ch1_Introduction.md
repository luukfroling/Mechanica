...
If friction can be ignored, the particle will reach the same height as it was released from, but now on the other side of the gutter. Galilei reasoned: if we widen the 'valley', still the particle will reach the same height if we ignore friction. Consequently, if we make the 'valley' infinitely wide, the particle will have to keep moving as eventually it should reach the other side and climb up. But then, in the horizontal part, its velocity cannot decrease as then it would ultimately come to a stand still and it will not reach the same height on the other side. This led Galilei to its law: if no force is applied to a particle, it will continue to move at a constant speed. This law is also know as the Law of Inertia.
